...
export interface ContractTypeEntityEvent {
...
export interface ContractTypeUpdateEntityEvent extends ContractTypeEntityEvent {
...
export class ContractTypeRepository {
    ...
    constructor(dataSource = "DefaultDB") {
        this.dao = daoApi.create(ContractTypeRepository.DEFINITION, undefined, dataSource);
    }
    ...
    public customDataCount(): number {
        const resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "CODBEX_CONTRACTTYPE"');
    ...
    private async triggerEvent(data: ContractTypeEntityEvent | ContractTypeUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-contracts-Settings-ContractType", ["trigger"]);
        ...
        producer.topic("codbex-contracts-Settings-ContractType").send(JSON.stringify(data));
    }
}
